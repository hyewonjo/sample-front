  ...
  <div class="board">
    ...
    <table>
      ...
      <tr v-for="boardItem in boardList" v-bind:key="boardItem.no">
        <td><span v-on:click="boardNoClick(boardItem)">{{boardItem.no}}</span></td>
<!--        router-link는 내부적으로 router.push() 메서드와 동일하게 처리됨.-->
        <td><router-link v-bind:to="{name: 'BoardView', query: {boardNo: boardItem.no}}">{{boardItem.subject}}</router-link></td>
...
export default {
  ...
  methods: {
    getBoardList() {
      this.axios.get("http://localhost:9000/boards")
          .then(res => {
